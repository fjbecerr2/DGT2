...
public class EnviarEmail{
	...
	private int mensajesOnsite =0; //Update 06/2022
	private int direccionesEmailIncorrectas =0;
	private int mensajesSolicitados =0;
	...
	private final String FILTROASUNTO ="Puesto de Trabajo/";
	/**
	* Clase que genera y envia el email con la url para la encuesta.
	*
	* @author Fco J. Becerra OTP-OPERACIONES NIVEL I
	...
	public boolean generarMensajeEmail(String[] datosEnvio, int[] contadores){
		...
		if(!error){ //Validad direcciones de to
			try {
				...
				emailAddr.validate();	
				//Update 06/2022
				if(datosEnvio[0].contains("onsite")){
					mensajesOnsite++;
					error =true;
				}else{
					GenerarLog.lineaLog("INFO","\t\t[TO] "+datosEnvio[0]);		
					//CORRECTIVO 06/2022 - Recortar el asunto
					
					int posicionAsunto =datosEnvio[2].indexOf("Puesto de Trabajo/")+FILTROASUNTO.length();
					String tipoPeticion =datosEnvio[2].substring(posicionAsunto);
					GenerarLog.lineaLog("INFO","\t\t[INCIDENCIA] "+datosEnvio[1]+" - "+tipoPeticion);
		...
		{	
			try{
				...
				int posicionAsunto =datosEnvio[2].indexOf("Puesto de Trabajo/")+FILTROASUNTO.length();
				String tipoPeticion =datosEnvio[2].substring(posicionAsunto);
				asunto +=" ["+datosEnvio[1]+"] - "+tipoPeticion;
	...
	*/
	public void verMensajesOKyKO(){
		GenerarLog.lineaLog("INFO","<EMAILS>");
		GenerarLog.lineaLog("INFO","--------");
		GenerarLog.lineaLog("INFO","\t["+this.mensajesOK+"]<ENVIADOS>");
		GenerarLog.lineaLog("INFO","\t["+this.mensajesKO+"]<NO ENVIADOS>");
		//Update 06/2022
		GenerarLog.lineaLog("INFO","\t["+this.mensajesOnsite+"]<DESCARTADOS ONSITE>");
		GenerarLog.lineaLog("INFO","\t["+this.direccionesEmailIncorrectas+"]<DIRECCIONES ERROR>");

		if((this.mensajesKO+this.mensajesOnsite)>this.mensajesOK){
			GenerarLog.lineaLog("WARNING","<BAJO NUMERO DE EMAILS ENVIADOS> ["+this.mensajesOK+"/"+(this.mensajesKO+this.mensajesOnsite)+"] sobre "+this.mensajesSolicitados);
